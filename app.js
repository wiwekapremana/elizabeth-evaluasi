...
function installQuestionnaireSubmitHandlers() {
  ...
  if (form) {
    // Jangan pernah biarkan form melakukan submit GET native.
    form.addEventListener("submit", (event) => {
      event.preventDefault();
      event.stopPropagation();
      openEvaluationConfirmation(event);
    });
  }

  if (button) {
    // Click normal bekerja di desktop dan mobile.
    // Jangan beri z-index ekstrem karena akan menutupi modal.
    button.type = "button";
